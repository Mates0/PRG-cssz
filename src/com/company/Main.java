...
public class Main {
    ...
    public static void main(String[] args) {
        String input = "2006,SO,Střední odborné vzdělání,615\n" +
        ...
        EmployeesEducation ee = new EmployeesEducation();

        String arr[] = input.split("\n");

        for (int i = 0; i < arr.length; i++) {
            if (arr[i].contains("2006") && arr[i].contains("Střední odborné vzdělání")) {
                String arr2[] = arr[i].split(",");
                ee.setSoCount(Integer.parseInt(arr2[3]));
            }
            if (arr[i].contains("2006") && arr[i].contains("Úplné střední odborné vzdělání")) {
                String arr2[] = arr[i].split(",");
                ee.setUsoCount(Integer.parseInt(arr2[3]));
            }
            if (arr[i].contains("2006") && arr[i].contains("Vysokoškolské vzdělání")) {
                String arr2[] = arr[i].split(",");
                ee.setVsCount(Integer.parseInt(arr2[3]));
            }
            if (arr[i].contains("2006") && arr[i].contains("Základní vzdělání")) {
                String arr2[] = arr[i].split(",");
                ee.setZakCount(Integer.parseInt(arr2[3]));
            }
        }

        System.out.println("---YEAR 2006---");
        System.out.println("ZAK: " + ee.getRatio("ZAK"));
        System.out.println("SO: " + ee.getRatio("SO"));
        System.out.println("USO: " + ee.getRatio("USO"));
        System.out.println("VS: " + ee.getRatio("VS"));

        for (int i = 0; i < arr.length; i++) {
            if (arr[i].contains("2020") && arr[i].contains("Střední odborné vzdělání")) {
                String arr2[] = arr[i].split(",");
                ee.setSoCount(Integer.parseInt(arr2[3]));

            }
            if (arr[i].contains("2020") && arr[i].contains("Úplné střední odborné vzdělání")) {
                String arr2[] = arr[i].split(",");
                ee.setUsoCount(Integer.parseInt(arr2[3]));
            }
            if (arr[i].contains("2020") && arr[i].contains("Vysokoškolské vzdělání")) {
                String arr2[] = arr[i].split(",");
                ee.setVsCount(Integer.parseInt(arr2[3]));
            }
            if (arr[i].contains("2020") && arr[i].contains("Základní vzdělání")) {
                String arr2[] = arr[i].split(",");
                ee.setZakCount(Integer.parseInt(arr2[3]));
            }
            if (arr[i].contains("2020") && arr[i].contains("Vyšší odborné vzdělání")) {
                String arr2[] = arr[i].split(",");
                ee.setVosCount(Integer.parseInt(arr2[3]));
            }
        }
        ...
        System.out.println("USO: " + ee.getRatio("USO"));
        System.out.println("VS: " + ee.getRatio("VS"));
        System.out.println("VOV: " + ee.getRatio("VOV"));
